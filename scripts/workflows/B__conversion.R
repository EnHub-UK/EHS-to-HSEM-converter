...
lbl_hsem <- c('dwelling.and.household.information', 'geometry',
              'ventilation','other.heat.loss.elements', 'space.heating',
              'hot.water.system', 'low.energy.lighting', 'complementary',
              'summarised', 'common')


# + Auxiliary Tables  ----------------------------------------------------------

tbl.AuxA <- load_auxiliary_table("tbl_CHM_EHS_ageBand")
tbl.AuxB <- load_auxiliary_table("tbl_SAPs16")
tbl.AuxC <- load_auxiliary_table("tbl_CHM_SAPs3", row.lbl=TRUE)
tbl.AuxD <- load_auxiliary_table("tbl_SAPs3", row.lbl=TRUE)
tbl.AuxE <- load_auxiliary_table("tbl_EHCS_heating")
tbl.AuxF <- load_auxiliary_table("tbl_HSEM_heating")
tbl.AuxG <- load_auxiliary_table("tbl_SAPs4b")
...
dtaModB$sap.no <- obtain_age_bands(dtaModB$fodconac)
...
dtaModC <- dtaDa
dtaModC <- dtaModC %>% na_if_fac() %>%
  mutate(hhsizex = hhsizex - NDEPCHILD)
colnames(dtaModC) <-
...
dtaFb <- dtaFb %>% na_if_num(88.8) %>% na_if_num(99.9) %>% na_if_fac()
dtaGb <- subset(flatdets, select=c(aacode, Fdfsamed, Fdfmainl,  Fdfnextl, Fdfmainw, Fdfmaind,  Fdfnextw, Fdfnextd, Fdffloor))
dtaGb <- dtaGb %>% mutate(Fdfsamed = log_fac(Fdfsamed)) %>%
  mutate(Fdfmainl = chr_fac(Fdfmainl)) %>%
  mutate(Fdfnextl = chr_fac(Fdfnextl)) %>%
  mutate(across(starts_with(c("Fdfmainl", "Fdfnextl")), ~as.character(.))) %>%
  na_if_num(77.7) %>% na_if_num(88.8) %>% na_if_num(99.9) %>% na_if_fac()
dtaHb <- subset(services, select=c(aacode, Finlopos))
dtaIb <- subset(interior, select=c(aacode, Finrooms, Finlivwi, Finlivde))
dtaIb <- dtaIb %>% na_if_num(88.8) %>% na_if_num(99.9)
...
dtaModA$storeyx <- convert_fac_to_int(dtaModA$storeyx)
...
dtaModB$storeyx <- convert_fac_to_int(dtaModB$storeyx)
...
dtaModC$Finrooms <- convert_fac_to_int(dtaModC$Finrooms)
...
dtaModD$sap.no <- obtain_age_bands(dtaModD$fodconac)
dtaModD$wall.th <- unlist(pblapply(dtaModD$aacode, obtain_thickness, dtaModD))
...
dtaModE <- dtaNb
dtaModE$Fvwtenff <- convert_fac_to_int(dtaModE$Fvwtenff)
dtaModE$Fvwtenbf <- convert_fac_to_int(dtaModE$Fvwtenbf)
dtaModE$Fvwtenlf <- convert_fac_to_int(dtaModE$Fvwtenlf)
dtaModE$Fvwtenrf <- convert_fac_to_int(dtaModE$Fvwtenrf)
...
dtaModF$AdditionalArea <- unlist(pblapply(dtaModF$aacode, obtain_addition, dtaModF))
...
dtaVb$Fexdf1no <- convert_fac_to_int(dtaVb$Fexdf1no)
...
dtaXb$Fexdf1no <- convert_fac_to_int(dtaXb$Fexdf1no)
...
dtaWb <- dtaRb
dtaWb$Felfenfw <- convert_fac_to_int(dtaWb$Felfenfw)
dtaWb$Felfenbw <- convert_fac_to_int(dtaWb$Felfenbw)
dtaWb$Felfenlw <- convert_fac_to_int(dtaWb$Felfenlw)
dtaWb$Felfenrw <- convert_fac_to_int(dtaWb$Felfenrw)
dtaWb[is.na(dtaWb)] <- 0
...
dtaZb <- dtaQb
dtaZb <- subset(dtaZb, select=c(aacode,Fdffroia,Fdfbckia,Fdflftia,Fdfrigia))
dtaZb$Fdffroia <- convert_fac_to_int(dtaZb$Fdffroia)
dtaZb$Fdfbckia <- convert_fac_to_int(dtaZb$Fdfbckia)
dtaZb$Fdflftia <- convert_fac_to_int(dtaZb$Fdflftia)
dtaZb$Fdfrigia <- convert_fac_to_int(dtaZb$Fdfrigia)
dtaZb[is.na(dtaZb)] <- 0
...
dtaAc <- dtaAc %>% mutate(across(starts_with(c("Type")), ~as.character(.))) %>% na_if_fac()
...
dtaCc <- dtaCc %>%
  mutate(Finlivle = chr_fac(Finlivle),
         Finkitle = chr_fac(Finkitle)) %>%
  mutate(across(starts_with(c("Finlivle", "Finkitle")), ~as.character(.))) %>%
  na_if_fac()
...
dtaModA$a.masonry <- convert_fac_to_int(dtaModA$a.masonry)
dtaModA$a.other <- convert_fac_to_int(dtaModA$a.other)
dtaModA$b.masonry <- convert_fac_to_int(dtaModA$b.masonry)
dtaModA$b.other <- convert_fac_to_int(dtaModA$b.other)
...
dtaModB <- dtaBc
dtaModB$Finchbcd <- suppressWarnings(as.integer(as.character(dtaModB$Finchbcd)))
dtaModB$Finchbcd <- ifelse(is.na(dtaModB$Finchbcd), 108, dtaModB$Finchbcd)
dtaModB$Finchbcd <- ifelse(dtaModB$Finchbcd=='9999'|dtaModB$Finchbcd=='7777'|dtaModB$Finchbcd=='8888', '108', dtaModB$Finchbcd)
dtaModB$heat.typ <- unlist(pblapply(dtaModB$aacode, obtain_heat_par, dtaModB))
dtaModB$heat.flue <- unlist(pblapply(dtaModB$aacode, obtain_heat_par, dtaModB, 'Boiler.Flue'))
...
lst.C <- as_tibble(join_all(
  lapply(ls()[grep("dtaMod[A-D]$",ls())], get), by='V001_HousingCode')) %>%
...
lst.D <- as_tibble(dtaModA) %>% mutate(across(where(is.character), ~na_if(., "-")))
rm(list=ls()[grep("dta\\w+",ls())])
message('part D .. done!')
...
dtaModA$heat.emitter <- unlist(pblapply(dtaModA$aacode, obtain_heat_par, dtaModA, 'Heat.emitter'))
dtaModA$heat.eff <- unlist(pblapply(dtaModA$aacode, obtain_heat_par, dtaModA, 'System.efficiency'))
...
lst.E <- as_tibble(dtaModA) %>%
  mutate(D083_MainHeatingElectricTariff = chr_fac(D083_MainHeatingElectricTariff),
         D084_MainHeatingCommunityHeatingTariff = chr_fac(D084_MainHeatingCommunityHeatingTariff),
         D085_MainHeatingCommunityHeatingFuel = chr_fac(D085_MainHeatingCommunityHeatingFuel),
         D087_MainHeatingCommunityHeatingCHPFuel = chr_fac(D087_MainHeatingCommunityHeatingCHPFuel)) %>%
  mutate(across(where(is.character), ~na_if(., "-")))
rm(list=ls()[grep("dta\\w+",ls())])
message('part E .. done!')
...
dtaModA$DHW.eff.est <- unlist(pblapply(dtaModA$aacode, obtain_heat_efficiency, dtaModA))
...
lst.F <- as_tibble(dtaModA) %>% mutate(across(where(is.character), ~na_if(., "-")))
rm(list=ls()[grep("dta\\w+",ls())])
message('part F .. done!')
...
dtaHa <- dtaHa %>% mutate(Fexdb1pr = log_fac(Fexdb1pr),Fexdb2pr = log_fac(Fexdb2pr)) %>% mutate(across(starts_with(c("type")), ~as.character(.))) %>% na_if_fac()
...
dtaHb <- dtaHb %>% mutate(across(where(is.factor), ~log_fac(.)))
...
dtaHd <- dtaHd %>% mutate(fincodor = log_fac(fincodor), fincorad = log_fac(fincorad)) %>%
  mutate(across(starts_with(c("fincowin", "fincorof")), ~as.character(.))) %>% na_if_fac()
...
dtaHf <- subset(detailed__energy_performance_plus, select=c(aacode, EPcalc09e, EPcshc09e, EPcwhc09e, EPcltc09e))
...
dtaHm <- subset(physical_plus, select=c(aacode,sap09,EPceeb09e,EPceir09e,EPceib09e,dhomesz,dhthermy))
...
dtaModA.c <- subset(dtaModA, select=c(aacode, type, Fexdb1ag))
dtaModA.c$Fexdb1ag <- suppressWarnings(convert_fac_to_int(dtaModA.c$Fexdb1ag))
dtaModA.c <- suppressMessages(dcast(dtaModA.c, aacode ~ type))
dtaModA.c$dormer.year <- rowMeans(dtaModA.c[,grep("aacode", colnames(dtaModA.c), invert = TRUE)], na.rm = T)
...
dtaModA.e <- subset(dtaModA, select=c(aacode, type, Fexdb2ag))
dtaModA.e$Fexdb2ag <- suppressWarnings(convert_fac_to_int(dtaModA.e$Fexdb2ag))
dtaModA.e <- suppressMessages(dcast(dtaModA.e, aacode ~ type))
...
dtaModA$fincosiz <- ifelse(is.na(dtaModA$fincodor) & is.na(dtaModA$fincosiz), dtaModA$AdditionalArea, dtaModA$fincosiz)
dtaModA <-
...
dtaModB$solar <- as.factor(paste(ifelse(!is.na(dtaModB$Felsolff)&dtaModB$Felsolff==TRUE,'F','-'),ifelse(!is.na(dtaModB$Felsollf)&dtaModB$Felsollf==TRUE,'L','-'),ifelse(!is.na(dtaModB$Felsolrf)&dtaModB$Felsolrf==TRUE,'R','-'),ifelse(!is.na(dtaModB$Felsolbf)&dtaModB$Felsolbf==TRUE,'B','-'),sep=""))
dtaModB$pv <- as.factor(paste(ifelse(!is.na(dtaModB$Felpvff)&dtaModB$Felpvff==TRUE,'F','-'),ifelse(!is.na(dtaModB$Felpvlf)&dtaModB$Felpvlf==TRUE,'L','-'),ifelse(!is.na(dtaModB$Felpvrf)&dtaModB$Felpvrf==TRUE,'R','-'),ifelse(!is.na(dtaModB$Felpvbf)&dtaModB$Felpvbf==TRUE,'B','-'),sep=""))
dtaModB$pot <- as.factor(paste(ifelse(!is.na(dtaModB$Fvwpvff)&dtaModB$Fvwpvff==TRUE,'F','-'),ifelse(!is.na(dtaModB$Fvwpvlf)&dtaModB$Fvwpvlf==TRUE,'L','-'),ifelse(!is.na(dtaModB$Fvwpvrf)&dtaModB$Fvwpvrf==TRUE,'R','-'),ifelse(!is.na(dtaModB$Fvwpvbf)&dtaModB$Fvwpvbf==TRUE,'B','-'),sep=""))
dtaModB$Finchbcd <- ifelse(dtaModB$Finchbcd==9999|dtaModB$Finchbcd==7777|dtaModB$Finchbcd==8888, 108, dtaModB$Finchbcd)
...
dtaModC <-
  dtaModC[,c('aacode','tenure8x','rumorph','rucontxt','Imd1010','hhtype11',
             ...
             'sexhrp','lenres','ALLincx','EPcalc09e', 'EPcshc09e',
             'EPcwhc09e', 'EPcltc09e')]
colnames(dtaModC) <-
  c('V001_HousingCode','V512_Tenure_8x','V513_Rurality','V514_Sparsity',
    ...
    'V525_IncomeCombined', 'V528_FuelExpenditure','V529_SpaceHeatCost',
    'V530_DHWCost', 'V531_LightingCost')
...
dtaModG <- dtaHm
colnames(dtaModG) <-
  c('V001_HousingCode', 'V592_SAPRating', 'V593_SAPRatingBand',
    'V594_SAPEnvironmentalRating', 'V595_SAPEnvironmentalRatingBand',
    'V596_DecentHomes', 'V597_DecentHomesThermalComfort')


dtaHha <- interior %>% select(aacode, Finlivex, Finlivle, Finlivfu, Finlivin, Finlivcl, Finlivwi, Finlivde) %>%
  mutate(Finlivex = log_fac(Finlivex), Finlivin = log_fac(Finlivin), Finlivle = chr_fac(Finlivle)) %>% na_if_num(88.8) %>% na_if_num(99.9) %>%
  select(aacode, Finlivle, Finlivfu, Finlivcl, Finlivwi, Finlivde)
dtaHhb <- interior %>% select(aacode, Finkitex, Finkitle, Finkitfu, Finkitin, Finkitcl, Finkitwi, Finkitde) %>%
  mutate(Finkitex = log_fac(Finkitex), Finkitin = log_fac(Finkitin), Finkitle = chr_fac(Finkitle)) %>% na_if_num(88.8) %>% na_if_num(99.9) %>%
  select(aacode, Finkitle, Finkitfu, Finkitcl, Finkitwi, Finkitde)
dtaHhc <- interior %>% select(aacode, Finbedex, Finbedle, Finbedfu, Finbedin, Finbedcl, Finbedwi, Finbedde) %>%
  mutate(Finbedex = log_fac(Finbedex), Finbedin = log_fac(Finbedin), Finbedle = chr_fac(Finbedle)) %>% na_if_num(88.8) %>% na_if_num(99.9) %>%
  select(aacode, Finbedle, Finbedfu, Finbedcl, Finbedwi, Finbedde)
dtaHhd <- interior %>% select(aacode, Finbatex, Finbatle, Finbatin, Finbatcl) %>%
  mutate(Finbatex = log_fac(Finbatex), Finbatin = log_fac(Finbatin), Finbatle = chr_fac(Finbatle)) %>% na_if_num(88.8) %>% na_if_num(99.9) %>%
  select(aacode, Finbatle, Finbatcl)
dtaHhe <- interior %>% select(aacode, Fincirex, Fincircl, Finstrpr) %>%
  mutate(Fincirex = log_fac(Fincirex), Finstrpr = log_fac(Finstrpr)) %>% na_if_num(88.8) %>% na_if_num(99.9)
dtaHhf <- interior %>% select(aacode, Finrooms, Finex1le, Finex1fu, Finex2le, Finex2fu, Finex3le, Finex3fu, Finex4le, Finex4fu, Finex5le, Finex5fu, Finex6le, Finex6fu, Finex7le, Finex7fu) %>%
  mutate(Finrooms = as.integer(Finrooms), Finex1le = chr_fac(Finex1le),Finex2le = chr_fac(Finex2le), Finex3le = chr_fac(Finex3le), Finex4le = chr_fac(Finex4le), Finex5le = chr_fac(Finex5le), Finex6le = chr_fac(Finex6le), Finex7le = chr_fac(Finex7le))

dtaModH <- join(dtaHha, dtaHhb, by='aacode')
dtaModH <- join(dtaModH, dtaHhc, by='aacode')
dtaModH <- join(dtaModH, dtaHhd, by='aacode')
dtaModH <- join(dtaModH, dtaHhe, by='aacode')
dtaModH <- join(dtaModH, dtaHhf, by='aacode')
dtaModH <- dtaModH %>% tibble()

colnames(dtaModH) <- c(
  'V001_HousingCode', 'V601_LivingRoom_Level', 'V602_LivingRoom_Function',
  'V603_LivingRoom_Height', 'V604_LivingRoom_Width', 'V605_LivingRoom_Depth',
  'V606_Kitchen_Level', 'V607_Kitchen_Function', 'V608_Kitchen_Height',
  'V609_Kitchen_Width', 'V610_Kitchen_Depth', 'V611_Bedroom_Level',
  'V612_Bedroom_Function', 'V613_Bedroom_Height', 'V614_Bedroom_Width',
  'V615_Bedroom_Depth', 'V616_Bathroom_Level', 'V616_Bathroom_Height',
  'V617_Circulation_Presence', 'V618_Circulation_Height', 'V619_Stairs_Within',
  'V620_Habitable_Rooms', 'V621_ExtraRoom_1_Level', 'V622_ExtraRoom_1_Function',
  'V623_ExtraRoom_2_Level', 'V624_ExtraRoom_2_Function', 'V625_ExtraRoom_3_Level',
  'V626_ExtraRoom_3_Function', 'V627_ExtraRoom_4_Level', 'V628_ExtraRoom_4_Function',
  'V629_ExtraRoom_5_Level', 'V630_ExtraRoom_5_Function', 'V631_ExtraRoom_6_Level',
  'V632_ExtraRoom_6_Function', 'V631_ExtraRoom_7_Level', 'V632_ExtraRoom_7_Function')

lst.H <- as_tibble(join_all(
  lapply(ls()[grep("dtaMod[A-H]$",ls())], get), by='V001_HousingCode')) %>%
  mutate(across(where(is.character), ~na_if(., "-")))
rm(list=ls()[grep("dta\\w+",ls())])
message('part H .. done!')
...
dtaModA <- dtaX %>%
  ...
  mutate(.attachL = ifelse(V580_CuboidAttachmentLeft==TRUE |
                             V579_CuboidAttachmentRear==TRUE, TRUE, FALSE),
         .attachR = ifelse(V581_CuboidAttachmentRight==TRUE |
                             V578_CuboidAttachmentFront==TRUE, TRUE, FALSE)) %>%
  mutate(.AttachState = ifelse(.attachL==T & .attachR==T, "B",
                        ifelse(.attachL==T & .attachR==F, "L",
                        ifelse(.attachL==F & .attachR==T, "R", "N")))) %>%
  mutate(code.floors = paste0(V574_CuboidFloors,'F'),
         code.attach = paste0(.AttachState,'A'),
         code.attic = ifelse(V576_CuboidAttic==T,'WA','NA'),
         code.cellar = ifelse(V577_CuboidBasement==T,'WB','NB')) %>%
  mutate(.cubdid = paste0("MOD-",code.floors,code.attic,code.cellar,code.attach),
         .heatid = obtain_heating_codeid(V001_HousingCode, dtaY, dtaW),
         .hubtyp = obtain_typology_fac('V583_CuboidType', dtaZ),
         .hubage = obtain_typology_fac('V584_CuboidEpoch', dtaZ)) %>%
  select(V001_HousingCode,.hubtyp,.hubage,.cubdid,.heatid)

lst.I <- as_tibble(dtaModA) %>% mutate(across(where(is.character), ~na_if(., "-")))
rm(list=ls()[grep("dta\\w+",ls())])
message('part I .. done!')
...
# J. [subset]-------------------------------------------------------------------
dtaMa <- subset(general_plus,
  select=c(aacode, tenure2x, tenure4x, GorEHS, region3x, vacantx))
dtaMb <- subset(physical_plus,
  select=c(aacode, dwtypenx, dwage5x, dwage6x, floor5x, attic, fuelx,
           dblglaz2, loftins4, heat4x, heat7x, watersys, boiler))
dtaMc <- subset(interview_plus,
  select=c(aacode, agehrp4x, agehrp6x, sexhrp, AHCinceq, hhinc5x,
           hhsizex, hhcompx, emphrpx, NDEPCHILD, ageoldbx, NBedsX))
...
lst.J <- as_tibble(dtaM) %>% mutate(across(where(is.character), ~na_if(., "-"))) %>%
  rename(V001_HousingCode = aacode)
rm(list=ls()[grep("dta\\w+",ls())])
message('part J .. done!')
